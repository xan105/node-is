...
function shouldError(value, error = null){
  if (check.isError(value) === false)
    throw asError(error) ?? new Failure(asString(error) ?? "Expecting an Error !", { code: 1, info: { type: typeof value, value: value } }); 
}

function shouldRegExp(value, error = null){
  if (check.isRegExp(value) === false)
    throw asError(error) ?? new Failure(asString(error) ?? "Expecting a RegExp !", { code: 1, info: { type: typeof value, value: value } }); 
}

...
function shouldString(value, error = null){
  if (check.isString(value) === false){
    ...
    throw asError(error) ?? new Failure(asString(error) ?? "Expecting a string !", { code: 1, info: { type: typeof value, value: value } }); 
  }
}

function shouldStringNotEmpty(value, error = null){
  if (check.isStringNotEmpty(value) === false) 
    throw asError(error) ?? new Failure(asString(error) ?? "Expecting a non-empty string !", { code: 1, info: { type: typeof value, value: value } });
}

function shouldHexString(value, error = null){
  if (check.isHexString(value) === false) 
    throw asError(error) ?? new Failure(asString(error) ?? "Expecting a hexadecimal string !", { code: 1, info: { type: typeof value, value: value } });
}

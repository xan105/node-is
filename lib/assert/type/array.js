...
function shouldArrayOfObjLike(value, schema, error = null){
  if (check.isArrayOfObjLike(value, schema) === false) 
    throw asError(error) ?? new Failure("Expecting an array of \"plain obj\" ({}, new Object(), Object.create(null)) like the given schema !", { code: 1, info: { type: typeof value, value: value, schema: schema } });
}

function shouldSizeArrayOfObjLike(value, length, schema, error = null){
  if (check.isSizeArrayOfObjLike(value, length, schema) === false) 
    throw asError(error) ?? new Failure(`Expecting an array[${length}] of "plain obj" ({}, new Object(), Object.create(null)) like the given schema !`, { code: 1, info: { type: typeof value, value: value, schema: schema } });
}

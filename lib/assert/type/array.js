...
function shouldArrayOfBuffer(value, error = null){
  if (check.isArrayOfBuffer(value) === false) 
    throw asError(error) ?? new Failure(asString(error) ?? "Expected array of Buffer !", { code: 1, info: { 
      type: typeof value, 
      tag: Object.prototype.toString.call(value), 
      value: value 
    }});
...
function shouldSizeArrayOfBuffer(value, length, error = null){
  if (check.isSizeArrayOfBuffer(value, length) === false) 
    throw asError(error) ?? new Failure(asString(error) ?? `Expected array[${length}] of Buffer !`, { code: 1, info: { 
      type: typeof value, 
      tag: Object.prototype.toString.call(value), 
      value: value 
    }});

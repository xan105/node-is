...
function shouldArrayOfNumberWithinRange(value, min, max, error = null){
  if (check.isArrayOfNumberWithinRange(value, min, max) === false) 
    throw asError(error) ?? new Failure(asString(error) ?? "Expecting an array of number between the given range !", { code: 1, info: { type: typeof value, value: value } });
}
...
function shouldSizeArrayOfNumberWithinRange(value, length, min, max, error = null){
  if (check.isSizeArrayOfNumberWithinRange(value, length, min, max) === false) 
    throw asError(error) ?? new Failure(asString(error) ?? `Expecting an array[${length}] of number between the given range !`, { code: 1, info: { type: typeof value, value: value } });
}
...
function shouldArrayOfIntegerWithinRange(value, min, max, error = null){
  if (check.isArrayOfIntegerWithinRange(value, min, max) === false) 
    throw asError(error) ?? new Failure(asString(error) ?? "Expecting an array of integer between the given range !", { code: 1, info: { type: typeof value, value: value } });
}
...
function shouldSizeArrayOfIntegerWithinRange(value, length, min, max, error = null){
  if (check.isSizeArrayOfIntegerWithinRange(value, length, min, max) === false) 
    throw asError(error) ?? new Failure(asString(error) ?? `Expecting an array[${length}] of integer between the given range !`, { code: 1, info: { type: typeof value, value: value } });
}
...
function shouldArrayOfObjWithProperties(value, prop, error = null){
  if (check.isArrayOfObjWithProperties(value, prop) === false) 
    throw asError(error) ?? new Failure(asString(error) ?? "Expecting an array of \"plain obj\" ({}, new Object(), Object.create(null)) containing the given properties !", { code: 1, info: { type: typeof value, value: value } });
}
...
function shouldSizeArrayOfObjWithProperties(value, length, prop, error = null){
  if (check.isSizeArrayOfObjWithProperties(value, length, prop) === false) 
    throw asError(error) ?? new Failure(asString(error) ?? `Expecting an array[${length}] of "plain obj" ({}, new Object(), Object.create(null)) containing the given properties !`, { code: 1, info: { type: typeof value, value: value } });
}

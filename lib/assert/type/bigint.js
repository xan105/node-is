...
function shouldBigInt(value, error = null){
  if (check.isBigInt(value) === false) 
    throw asError(error) ?? new Failure("Expecting a BigInt !", { code: 1, info: { type: typeof value, value: value } }); 
}

function shouldBigIntPositive(value, error = null){
  if (check.isBigIntPositive(value) === false) 
    throw asError(error) ?? new Failure("Expecting a BigInt > 0 !", { code: 1, info: { type: typeof value, value: value } });
}
...
function shouldBigIntWithinRange(value, min, max, error = null){
  if (check.isBigIntWithinRange(value, min, max) === false) 
    throw asError(error) ?? new Failure("Expecting a BigInt between the given range !", { code: 1, info: { type: typeof value, value: value, range: [min,max] } });
}

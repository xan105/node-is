...
function shouldIP(string, error = null){
  if (check.isIP(string) === false) 
    throw asError(error) ?? new Failure("Expecting a valid IP address !", { code: 2, info: { value: string } });
}

function shouldIPv4(string, error = null){
  if (check.isIPv4(string) === false) 
    throw asError(error) ?? new Failure("Expecting a valid IPv4 address !", { code: 2, info: { value: string } });
}

function shouldIPv6(string, error = null){
  if (check.isIPv6(string) === false) 
    throw asError(error) ?? new Failure("Expecting a valid IPv6 address !", { code: 2, info: { value: string } });
}

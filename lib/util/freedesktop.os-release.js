...
async function parse(){ //Modified from @xan105/ini [github.com/xan105/node-ini] (MIT) since it's an .ini like syntax
  ...
  for (const line of lines){
    ...
    if (!line || line.match(/^\s*[#]/)) continue; //ignore empty and comment
    ...
    /*
      ...
      spaces, semicolons or other special characters outside of A–Z, a–z, 0–9. 
      (Assignments that do not include these special characters may be enclosed in quotes too, but this is optional.)
    */
    const value = unquote(line.slice(pos + 1).trim()); //unquote
